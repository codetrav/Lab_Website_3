...
var game_data = 'select game_date,visitor_name,home_score,visitor_score FROM football_games'
...
var player_data = 'select * FROM football_players'
db.task('get-everything', task => {
    return task.batch([
        task.any(player_data),
    ]);
})
.then(data => {
...
sel_player_data = 'SELECT * FROM football_players WHERE id = ' + player_id;
cnt_games = 'SELECT COUNT(*) FROM football_games WHERE ' + player_id +'=ANY(players)';
player_data = 'SELECT * FROM football_players';
db.task('get-everything', task => {
    return task.batch([
        task.any(player_data),
        task.any(sel_player_data),
        task.any(cnt_games)
    ]);
})
.then(data => {
